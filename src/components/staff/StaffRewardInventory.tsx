                        ...
                        <section className="mb-10">
                            ...
                            <div className="flex flex-col sm:flex-row items-stretch gap-2 sm:gap-3 mb-5 sm:max-w-xl">
                                <div className="relative w-full">
                                    ...
                                    <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search rewards..." className="w-full pl-9 pr-9 py-2 rounded-lg border border-gray-300 bg-white text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-green-400 focus:border-transparent transition" />
                                    {search && (
                                ...
                                <div className="relative w-full sm:w-36 shrink-0">
                                    ...
                                    <select value={stockFilter} onChange={(e) => setStockFilter(e.target.value as "all" | "instock" | "outofstock")} className="w-full pl-9 pr-8 py-2 rounded-lg border border-gray-300 bg-white text-sm font-medium text-gray-700 appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-green-400 transition">
                                        <option value="all">All</option>
                                        <option value="instock">In Stock</option>
                                        <option value="outofstock">Out of Stock</option>
                                    </select>
                                    <span className="absolute inset-y-0 right-3 flex items-center pointer-events-none">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                                    </span>
                                </div>
                            </div>
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody className="bg-white divide-y divide-gray-100 text-sm text-gray-700">
                                        {isLoading ? (
                                            ...
                                            <tr><td colSpan={7} className="text-center py-6 text-red-500 italic">Failed to load rewards. Please try again later.</td></tr>
                                        ) : !rewards || rewards.length === 0 ? (
                                            <tr><td colSpan={7} className="text-center py-6 text-gray-500 italic">No rewards in inventory.</td></tr>
                                        ) : sortedRewards.length === 0 ? (
                                            <tr><td colSpan={7} className="text-center py-6 text-gray-400 italic">No rewards match &ldquo;{search}&rdquo;.</td></tr>
                                        ) : (
                                            paged.map((reward, index) => (
                                                <tr key={reward.id} className="hover:bg-gray-50 transition">
                                                    ...
                                                    <td className="px-4 py-3">
                                                        {reward.image ? (
                                                            <img src={reward.image} alt={reward.name} className="w-10 h-10 rounded-lg object-cover border border-gray-200" />
                                                        ) : (
                                                    ...
                                                    <td className="px-4 py-3 text-center">
                                                        <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${reward.stock <= 0 ? "bg-red-100 text-red-600" : reward.stock <= 5 ? "bg-yellow-100 text-yellow-700" : "bg-green-100 text-green-700"}`}>
